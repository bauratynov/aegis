 ...
 */
import { signal, computed, effect, html, list, show, mount, on, interval, dev, stats, onWarn, trace, destroy, cache, invalidate } from './aegis.js';
...
.tabs { display: flex; gap: 2px; padding: 4px 6px; border-bottom: 1px solid #30363d } .tabs button { flex: 1; background: none; border: 1px solid transparent; color: #8b949e; border-radius: 6px; padding: 2px; cursor: pointer; font: inherit } .tabs button.on { color: #58a6ff; border-color: #30363d; background: #161b22 }
.ctable { width: 100%; border-collapse: collapse; font-size: 11px } .ctable td, .ctable th { padding: 2px 4px; text-align: left; border-bottom: 1px solid #21262d; white-space: nowrap } .ctable th { color: #8b949e; font-weight: normal }
.ctable td.k { max-width: 190px; overflow: hidden; text-overflow: ellipsis } .ctable tr.fresh td.s { color: #3fb950 } .ctable tr.stale td.s { color: #d29922 } .ctable tr.error td.s { color: #f85149 } .ctable tr.inflight td.s { color: #58a6ff }
.ctable button { background: none; border: 0; color: #8b949e; cursor: pointer; font: inherit; padding: 0 3px } .ctable button:hover { color: #58a6ff }
.bar { display: inline-block; height: 4px; width: 40px; background: #21262d; border-radius: 2px; vertical-align: middle } .bar i { display: block; height: 100%; background: #3fb950; border-radius: 2px }
`;
...
export function open() {
    ...
    const api = mount(root, ({ signal, computed, effect, html, list, show, on, interval }) => {
        ...
        const cs = computed(() => { tick.value; return tab.value === 'cache' ? cache.stats() : null; });
        const fmtAge = (ms) => ms == null ? '—' : ms < 1000 ? ms + 'ms' : ms < 60000 ? Math.round(ms / 1000) + 's' : Math.round(ms / 60000) + 'm';
        const fmtBytes = (n) => n < 0 ? '?' : n < 1024 ? n + 'B' : n < 1048576 ? (n / 1024).toFixed(1) + 'K' : (n / 1048576).toFixed(1) + 'M';
        const explain = (key) => { const x = cache.explain(key); console.log('[Aegis cache] ' + key + ': ' + x.state + ' — ' + x.why); console.table(x.history); };
        return html`
            <div class=${{ panel: true, min: minimized }}>
                ...
                <div class="tabs">
                    <button class=${{ on: () => tab.value === 'components' }} @click=${() => { tab.value = 'components'; }}>components</button>
                    <button class=${{ on: () => tab.value === 'cache' }} @click=${() => { tab.value = 'cache'; }}>cache ${() => s.value.resourceCache}</button>
                </div>
                ${show(() => tab.value === 'cache', () => html`<div class="detail">
                    <div class="stats">
                        <div><b>${() => cs.value ? cs.value.entries.length : 0}</b><span>entries</span></div>
                        <div><b>${() => cs.value ? cs.value.entries.filter(x => x.subscribers > 0).length : 0}</b><span>live</span></div>
                        <div><b>${() => cs.value ? cs.value.prefetch.used + '/' + cs.value.prefetch.fired : '0/0'}</b><span>prefetch used</span></div>
                        <div><b>${() => cs.value ? fmtBytes(cs.value.entries.reduce((a, x) => a + Math.max(0, x.size), 0)) : 0}</b><span>data</span></div>
                    </div>
                    <table class="ctable"><thead><tr><th>key</th><th>state</th><th>age / stale</th><th>subs</th><th>size</th><th>fetches</th><th></th></tr></thead>
                    <tbody>${list(() => cs.value ? cs.value.entries : [], (r) => html`<tr class=${r.state}>
                        <td class="k" title=${r.key}>${r.key}</td><td class="s">${r.state}</td>
                        <td><span class="bar"><i style=${{ width: (r.staleTime ? Math.min(100, (r.age || 0) / r.staleTime * 100) : 100) + '%' }}></i></span> ${fmtAge(r.age)}/${fmtAge(r.staleTime)}${r.suggestedStaleTime ? ' → ' + fmtAge(r.suggestedStaleTime) : ''}</td>
                        <td>${r.subscribers}</td><td>${fmtBytes(r.size)}</td><td>${r.fetches}${r.unchanged ? ' (' + r.unchanged + '=)' : ''}</td>
                        <td><button title="invalidate" @click=${() => invalidate(r.key)}>↻</button><button title="remove" @click=${() => cache.remove(r.key)}>×</button><button title="explain in console" @click=${() => explain(r.key)}>?</button></td></tr>`, { key: 'key' })}</tbody></table>
                    ${show(() => cs.value && cs.value.entries.length === 0, () => html`<div class="empty">cache is empty</div>`)}
                </div>`)}
                <div class="body" hidden=${() => tab.value !== 'components'}>
                    <div class="tree">

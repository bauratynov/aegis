...
The node suite covers the signal graph only. The DOM, templates, islands, lists and morph, the router, forms, the cache and offline are covered by the browser suite: `test/test.html`, 1 085 assertions in about 50 sections, run in headless Chrome, Firefox and WebKit by `npm run test:browsers`. A green node run says the core is right; it says nothing about the UI layer.

...
| Code | What | Fix |
|---|---|---|
| **E001** | `effect()` created outside a scope — it will never be cleaned up. The effect still runs: this is a reported leak, not a prevented one. | Create it inside `island()`/`mount()` setup or `scope.run(() => …)`; `window.__AEGIS_DEV__ = 'strict'` throws here instead of warning; `defaults.orphanEffects = 'throw'` throws in production too, `'root'` adopts the effect into an app-level scope. |

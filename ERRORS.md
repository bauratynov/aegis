...
| **E027** | A flush took many rounds — effects keep writing signals other effects depend on (ping-pong). Since phase A1 the scheduler learns writer → reader edges: a self-write (`s.value = s.value + 1` inside the effect that reads `s`) and a write cycle between effects are reported in the first round with the effect names, not after the round limit. | Derive with `computed()` instead of writing from an effect; batch related writes; break the cycle. Applies to `flush: 'micro'` / `'frame'` lanes too: a lane drains in one tick, a self-loop throws `Infinite reactive loop in "micro" lane`. |

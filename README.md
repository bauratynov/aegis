...
One ES module, no build step, no bundler required. Django, Laravel, Rails, Go, PHP: keep your templates, add <code>data-aegis</code> where the page has to react.</p>
...
<p align="center">
  ...
  <a href="https://www.npmjs.com/package/@aegisjs/engine"><img src="https://img.shields.io/npm/v/@aegisjs/engine" alt="npm"></a>
...
<script type="module">
import { island } from 'https://cdn.jsdelivr.net/npm/@aegisjs/engine@0.7.2/aegis.min.js';
...
- **Removed in 0.7.1:** the old aliases `clsMap`, `store`, `cachedResource`, `offlineResource` and `component` are gone; `cls(el, { … })`, `reactive(obj, { shallow: true })`, `resource(url, { cache: true })`, `resource(url, { offline: true })` and `mount()` are the names. Nothing in the file is marked `@deprecated`.
...
{
  "imports": {
    "aegis": "https://cdn.jsdelivr.net/npm/@aegisjs/engine@0.7.2/aegis.min.js",
    "aegis/core": "https://cdn.jsdelivr.net/npm/@aegisjs/engine@0.7.2/aegis.core.min.js",
    "aegis/test": "https://cdn.jsdelivr.net/npm/@aegisjs/engine@0.7.2/aegis-test.js"
  },
  "integrity": { "https://cdn.jsdelivr.net/npm/@aegisjs/engine@0.7.2/aegis.min.js": "sha384-33WGGKOW1cXvGQODmX6XMy0kRCb00Gkgy4P5vJ08J+mqES1wy9UbLCjKtqAbhvhU" }
}
</script>
<script type="module">
...
The block above pins the npm release on jsDelivr, which serves it the moment it is published; `dist/importmap.html` in this repository carries the same three files with `modulepreload` and hashes, regenerated by `build.mjs` on every version. The site mirrors each release at `https://aegisjs.com/<version>/` once it is deployed, and `https://aegisjs.com/aegis.js` without a version is the latest release, for experiments only. The hashes of every file: [`dist/integrity.json`](https://github.com/bauratynov/aegis/blob/main/dist/integrity.json) and [aegisjs.com/docs/installation](https://aegisjs.com/docs/installation/).
...
The pinned URL is the whole file, so a page that imports five names still downloads 108 KB. If that matters, build the subset once (below) or let a bundler tree-shake it; the per-subset numbers are in [`docs/distribution.md`](https://github.com/bauratynov/aegis/blob/main/docs/distribution.md).

The production numbers assume the dev layer is dropped: `build.mjs` does it for you, a bundler needs `define: { 'globalThis.AEGIS_PROD': 'true' }` (esbuild, Vite, Rollup, webpack all spell it similarly). Without that define the same subsets weigh 11 KB and 38 KB instead of 7 and 30 — the difference is the 60+ dev warnings.
...
# → one file with exactly the exports admin.js imports (a 14-export admin bundle is 22 KB gzip); unknown names fail here, not in the browser
```

### npm

The package is `@aegisjs/engine` — the scope matches the site, and the bare names are taken: `aegis` and `aegis-engine` belong to other projects, and `aegisjs` is refused by npm as too close to `aegis-js` (last published 2022).

```sh
npm i aegis@npm:@aegisjs/engine   # keeps `from 'aegis'` working under a bundler
npm i @aegisjs/engine             # or the real name, then import from '@aegisjs/engine'
```

The `exports` map (`.`, `./min`, `./core`, `./core/min`, `./devtools`, `./test`) resolves the same subpaths either way. The pinned URL above stays the release channel for pages without a bundler.

### How the bare name `aegis` resolves

The examples on the site import `from 'aegis'` (in this repository they import `./aegis.js`, so that a clone runs with no import map at all). That name is not magic: it comes from the import map in your base template (the pinned block above). With a bundler it comes from `node_modules` instead — install under that name with `npm i aegis@npm:@aegisjs/engine`, or import `'@aegisjs/engine'` directly. Without either, import the URL directly:
